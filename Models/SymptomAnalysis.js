...
symptomAnalysisSchema.pre('save', function(next) {
  if (this.severity == null || this.severity === '') {
    if (this.confidence < 0.3) this.severity = 'low';
    else if (this.confidence < 0.7) this.severity = 'medium';
    else if (this.confidence < 0.9) this.severity = 'high';
    else this.severity = 'critical';
  }
  if (this.isEmergency == null) {
    const criticalDiseases = ['heart attack', 'stroke', 'severe allergic reaction', 'pneumonia', 'meningitis', 'sepsis'];
    this.isEmergency = criticalDiseases.some(d => this.predictedDisease && this.predictedDisease.toLowerCase().includes(d));
    if (this.isEmergency) this.severity = 'critical';
  }
  next();
});
